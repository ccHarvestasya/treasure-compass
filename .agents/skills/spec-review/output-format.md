...
- 保留した指摘: Design / Implementation / Test 等の downstream、後続検証、または現在の根拠では評価できず後続確認が必要な未決定事項・確認事項への引継ぎ。単に現在の対象範囲外である事項は原則として含めず、必要な場合だけレビュー対象または対象範囲と追跡に記録する。上流の正式資料へのフィードバックは含めない
