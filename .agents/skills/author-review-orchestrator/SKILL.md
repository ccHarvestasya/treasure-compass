...
`REVISION_NEEDED` の場合は、同じ Author が指摘に必要な範囲だけを修正し、同じ Reviewer が再確認する。`CREATE_COMMIT_REVIEW` では修正を対象変更の追加コミットとして記録し、Reviewer は最新の対象コミットをレビューする。履歴を書き換える操作は、ユーザーが明示しない限り行わない。
