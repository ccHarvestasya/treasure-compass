...
- 必須修正: `CRITICAL` / `HIGH` の New / Open / Reopened。1件以上あれば `REVISE IMPLEMENTATION`
- 任意改善: `MEDIUM` / `LOW` の New / Open / Reopened。これらのみなら `READY` とできる
- 上流へのフィードバック: 上流の不足・曖昧さ・未決定は `Upstream Feedback` として記録し、発生源を `Upstream ambiguity`、`Specification gap`、`Design gap` に分ける。Implementation defect として二重計上せず、上流から新しい契約・設計・要求を作らない
- 保留した指摘: 後続検証、外部環境、運用、release、または後続工程で確認すべき事項への引継ぎ。単に現在の対象範囲外である事項や上流問題は含めない
