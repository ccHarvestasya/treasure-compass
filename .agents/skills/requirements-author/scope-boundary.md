...
3. 採用すると、入力資料にない機能、利用者、環境、環境、実行環境、責任、互換性、運用を追加しないか。
...
- 利用者、関係者、外部主体の責任
- 対象・対象外・環境・環境・実行環境の境界
- 機能の目的と外部から観測可能な結果
- security、privacy、完全性、認証、可用性、相互運用性などの必要特性
- 運用、法務、license、外部連携の不可欠な制約
- 受け入れ条件、失敗時の安全な結果、未決定事項

API、field、型、データ保護方式、保存方式、入力識別、データ形式、database、framework、画面、テストケースを具体化する候補は、この要件定義では採用しない。
