...
4. **Public API / public contract / compatibility**: TypeScript、TypeScript、browser、build、公開境界 の API、型、ownership、互換性が一致する。
5. **Distribution contents**: crate、package、browser、build、公開境界 archive に必要な物だけが含まれ、入力データ や不要な開発物がない。
6. **Platform / runtime support**: supported target、baseline、browser / browser routing、browser fallback、failure path が証拠で裏付けられる。
7. **Security / 入力データ handling**: 入力データ handling、入力処理、export、security guarantee、fail-closed boundary が過剰記載や漏えいなく説明される。
...
13. **Public hygiene**: obsolete wording、placeholder、local path、内部 reference、誤った metadata / copyright、unsupported claim が公開面に残らない。
